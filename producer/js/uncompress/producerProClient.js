...
var replayTimer = "",setimeout=200,dataArray,timeBase = {},pts0 = {};
function setChannelEvents(channel,channelTag,peerName_,pc) {
    channel.onmessage = function (event) {
        ...
        if (!pts0[peerName_]) {
            pts0[peerName_] = obj.ts;
            timeBase[peerName_] = Date.now();
        }
        // if($("#preview .main-player").attr("id")==peerName_){
        //     console.log(event);
        // }
        var arr = peerClientStatusArr.dataArray[peerName_];
        if (arr.length == 0){
            // if(arr.length==0){
            //     arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
            // }else {
            //     for(var i = arr.length-1; i >= 0; i--){
            //         if(parseInt(arr[i]["pts"])<obj.ts){
            //             arr.splice(i, 0,{pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
            //             break;
            //         }else{
            //             arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
            //             break;
            ...
            // }
            arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
        } else if(obj.ts - arr[0].pts < 3000) {
            arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
        } else {
            // var arr = peerClientStatusArr.dataArray[peerName_]; 
            // for(var i = arr.length -1; i >= 0; i--){
            //     if(parseInt(arr[i]["pts"])<obj.ts){
            //         peerClientStatusArr.dataArray[peerName_].splice(i, 0,{pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
            //         break;
            //     }else{
...
PeerClient.prototype.onRemoteStreamAdded = function(event) { //成功创建连接，获取到视频
    ...
    if(currentRInfo.rtcIdList.pvw.indexOf(name)>-1||currentRInfo.rtcIdList.pgm.id.indexOf(name)>-1){
        var div ='<video  autoplay playsinline class="left main-player" id="'+name+'"></video>';
        var videoBox = null;
        if(currentRInfo.rtcIdList.pvw.indexOf(name)>-1){
            videoBox = videoDom.input;
            // div += '<div class="timeStamp"></div><div class="currentTime1"></div><div class="currentTime2"></div><div class="currentTime"></div>';
        }else{
        ...
        if(currentRInfo.rtcIdList.pvw.indexOf(name)>-1){
            ...
            // $(".main-preview .main-player")[0].ontimeupdate = function() {
            //     updatepts();
            // }
            // function updatepts(){
            //     var currentTime = $(".main-preview .main-player")[0].currentTime;
            //     $("#preview .currentTime").html(currentTime);
            // }
        ...
        }else if(curItem.PreviewShm.SourceType==600){
            sourceTypeIcon = "icon-quadView";
        }else{
            sourceTypeIcon = "icon-pack";
            if(statusObj.isVoIpFunction) var voIpIcon = "icon-msnui-telephone"; 
        }
        var previewListItem =   '<li class="preview-item '+isR+'" data-filename="'+curItem.PreviewShm.SharedMemoryName+'" '+rid+'>\
                                    ...
                                    <div class="preview-item-control clearFix">\
                                        ...
                                        <div class="item-control-voice erphone right">\
                                            <span class="iconfont voip-calling '+voIpIcon+'"></span>\
        ...
        var eleRArr = [],eleOtherArr=[],mainR='',rpsArr=[];
        ...
        if(eleRArr.length!=0)eleRArr = eleRArr.sort(sortVideo);
        if(rpsArr.length!=0) eleRArr = rpsArr.sort(sortRpsVideo);
        ...
        if(volume.volumeColumn.volumeObj[curItem.PreviewShm.SharedMemoryName]){
            curItem.CurrentVolume = volume.volumeColumn.volumeObj[curItem.PreviewShm.SharedMemoryName]["CurrentVolume"];
        }else{
            curItem.CurrentVolume = 0;
        }
        var previewVolume = volume.volumeConversion.volumeR2local(curItem.CurrentVolume);
        if(previewVolume > 0){
        ...
        if(previewVolume>100||previewVolume<0){
            console.log(volume.volumeColumn.volumeObj[curItem.PreviewShm.SharedMemoryName]["CurrentVolume"]);
        }
        volume.createVoiceControl(previewVolume, $('.preview-content [data-filename="'+curItem.PreviewShm.SharedMemoryName+'"]'),'vertical');
    ...
    }else if(statusObj.voIpFlag){
        var div = document.createElement("div");
        div.setAttribute("id", name);
        div.setAttribute('class','singleViopDiv');
        var a = document.createElement("AUDIO");
        a.setAttribute("autoplay", "autoplay");
        a.srcObject = event.stream;
        div.appendChild(a);
        console.log(div);
        var voipAudioBox = $(".voip-calling[data-peertid='"+name+"']");
        voipAudioBox.append(div); //生成媒体
        var voId=name.slice(2).toLowerCase();
        $(".voip-calling[data-rlivepeerid='"+ voId +"']").addClass('Connected');
        $(".voip-calling[data-rlivepeerid='"+ voId +"']").removeClass("loading-voip-color");
        $(".voip-calling[data-rlivepeerid='"+ voId +"']").addClass('icon-msnui-telephone');
    }
};
...
function sortRpsVideo(ele1,ele2){
    var id1 = parseInt($(ele1).attr("data-filename").slice(-1)),
        id2 = parseInt($(ele2).attr("data-filename").slice(-1));
    if(id1<id2){
        return -1;
    }else if(id1>id2){
        return 1;
...
var RtcClient = function (params,url) {
    this.logIn = false;
    ...
    this.localStream_ = {};
    //signaling
    this.sigChannel_ = new Signaling(url);
    this.initSigChannel();
    /*if (this.peerid_) {
...
RtcClient.prototype.onSigServerConnected = function () {
    ...
    //把当前的发送创建重新创建一遍
    if(currentRInfo.rtcIdList.pvw.length!=0 || currentRInfo.rtcIdList.pgm.id.length!=0){
        //如果这两个有值，发送
        if (currentRInfo.rtcIdList.pvw.length != 0){
            on_rtc(currentRInfo.rtcIdList.pvw);
            return false;
        }
        on_rtc(currentRInfo.rtcIdList.pgm.id);    
    }else if (currentRInfo.rtcIdList.preLis.idArr.length != 0){
        //如果是preview_list中有值
        on_rtc(currentRInfo.rtcIdList.preLis.idArr);
    } else if (peerClientStatusArr.resetVideoList.length != 0){
        on_rtc(peerClientStatusArr.resetVideoList);
    }
};
...
function errorMsg(error) {
    ...
    console.log(err);
    if(statusObj.voIpFlag){
        if(error.message.indexOf("Failed to get access to local media")>-1){
            oUtils.alertTips("i18n_VOIPUserMediaFailed",2000);
            error.name=error.name[0];
        }else if(error.message.indexOf("Busy!")>-1||error.message == "Peer refuse call request"||error.message == "peer is busy"){
            oUtils.alertTips("i18n_callConnectionFail");
        }
        statusObj.voIpFlag = false;
        var voId=error.name.slice(2).toLowerCase();
        var rid=currentRInfo.voIpTlist[voId];
        $(".preview-item[data-rid='"+ rid +"']").find(".voip-calling").removeClass('Connected');
        $(".preview-item[data-rid='"+ rid +"']").find(".voip-calling").addClass('icon-msnui-telephone');
        $(".preview-item[data-rid='"+ rid +"']").find(".voip-calling").removeClass("loading-voip-color");
        return false;
    }
    if(err.indexOf("Ice connection failed")>-1){
        ...
        if(peerClientStatusArr.disconnectNum>=2){
            oUtils.alertTips("i18n_webRTCOffline");
            initSystem.webrtcVideo.initFlag= true;
            ...
            if(currentRInfo.rtcIdList.pvw.indexOf(error.name)>-1||currentRInfo.rtcIdList.pgm.id.indexOf(error.name)>-1){
                ...
                peerClientStatusArr.videoResetNumObjTime = setTimeout(function(){
                    ...
                    for(var j=0;j<userNotExists.length;j++){
                        var index = initSystem.webrtcVideo.peerIds.indexOf(userNotExists[j])
                        if(index>-1){
                            on_leave(userNotExists[j]);
                            initSystem.webrtcVideo.peerIds.splice(index, 1);
    ...
    if(err.indexOf("already exists")>-1){
        var alreadyIndex = initSystem.webrtcVideo.peerIds.indexOf(error.name);
        if(alreadyIndex>-1){
            on_leave(error.name);
            initSystem.webrtcVideo.peerIds.splice(alreadyIndex, 1);
        }
    }
}
...
RtcClient.prototype.onUserMediaError = function(error,peerIdArr) {
    var errorMessage = 'Failed to get access to local media. Error name was ' + error.name ;
    errorMsg({name:peerIdArr, message:errorMessage});
...
RtcClient.prototype.onLogIn = function (msg,peerIdArr) {
   //var ack = JSON.parse(msg);
    if (msg.success) {
        ...
        if (this.needCallAfterLogIn &&this.callee_ != null) {
            ...
            if(statusObj.voIpFlag){
                this.mediaConstraints_.audio = true;
                 ...
                 .then( function(stream) {
                    ...
                    this.onUserMediaError(error,peerIdArr);
                }.bind(this));
            }else{
...
function calPts(id) {
    ...
    //     if (timeBase[id] + dataArray[i].pts - pts0[id] > currentT - dataArray[i].tarDelay){
    //         if (i===0) {
    ...
    //     return dataArray[dataArray.length-1].pts;
    // }
    var i,currentT = Date.now(); 
    var dataArray = peerClientStatusArr.dataArray[id]; 
    if (dataArray.length === 0) { 
    ...
    for (i = 0; i < dataArray.length; i++) { 
        // currentT - dataArray[i].rt - dataArray[i].tarDelay < 1000 / dataArray[i].fps
        // if(dataArray[i].rt > currentT - dataArray[i].tarDelay) { 
        if(currentT - dataArray[i].rt - dataArray[i].tarDelay < 1000 / dataArray[i].fps) { 
            return dataArray[i].pts; 
        } 
    } 
    if (i === dataArray.length) { 
...
function on_rtc (peerIdArr,previewList) {
    ...
    if (peerIdArr.length) {
        ...
        window.rtcclient.onLogIn({success:true},peerIdArr);
    } else {
        err = 'Callee is empty';
        errorMsg({name:'user', message:err});
    };       
...
// setInterval(function(){
    ...
    // if(peerClientStatusArr.dataArray[id]&&peerClientStatusArr.dataArray[id].length>0){
        // var pts = calPts(id);
        // $("#preview .timeStamp").html(pts + "  " + pts0[id] + "  " + timeBase[id]);
