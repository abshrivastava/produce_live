...
function setChannelEvents(channel,channelTag,peerName_,pc) {
    channel.onmessage = function (event) {
        ...
        if (arr.length == 0){
            ...
            arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST, dur:0});
        } else if(obj.ts - arr[0].pts < 3000) {
            var dur = obj.ts - arr[arr.length - 1].pts;
            arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: arr[arr.length - 1].rt + dur,fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST,dur:dur});
        } else {
            // var arr = peerClientStatusArr.dataArray[peerName_]; 
            // for(var i = arr.length -1; i >= 0; i--){
            //     if(parseInt(arr[i]["pts"])<obj.ts){
...
function calPts(id) {
    ...
    for (i = 0; i < dataArray.length; i++) {
        if(currentT - dataArray[i].rt - dataArray[i].tarDelay < dataArray[i].dur) {
