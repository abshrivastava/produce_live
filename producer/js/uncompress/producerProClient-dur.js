...
function setChannelEvents(channel,channelTag,peerName_,pc) {
    channel.onmessage = function (event) {
        ...
        if (arr.length == 0){
            ...
            var dur = obj.ts - arr[arr.length - 1].pts;
            arr.push({pts: obj.ts, curDelay: peerClientStatusArr.backData[peerName_].delayMSC, rt: arr[arr.length - 1].rt + dur,fps:peerClientStatusArr.backData[peerName_].frameRate,tarDelay: peerClientStatusArr.backData[peerName_].delayMST});
        } else {
            // var arr = peerClientStatusArr.dataArray[peerName_]; 
            // for(var i = arr.length -1; i >= 0; i--){
            //     if(parseInt(arr[i]["pts"])<obj.ts){
            ...
            // }
            var dur = obj.ts - arr[arr.length - 1].pts;
            // if (Math.abs(arr[arr.length - 1].rt + dur - Date.now()) > dur) {
...
function calPts(id) {
    /*
        if (peerClientStatusArr.dataArray[peerName_].length < 60){
    ...
    var currentDelay  = 0;
    for(var j = 1;j <= 10; j++){
        currentDelay += parseInt(dataArray[dataArray.length - j].tarDelay);
    }
    currentDelay /= 10;
    // for (i = 0; i < dataArray.length; i++) {
    //     if(currentT - dataArray[i].rt - peerClientStatusArr.backData[id].delayMST < 1000 / dataArray[i].fps / 2) {
    //         // for (var k = i + 1; k<dataArray.length; k++){
    //         //     // console.log(currentT - dataArray[k].rt - dataArray[k].tarDelay + currentDelay/5, dataArray[i].fps)
    //         //     if(currentT - dataArray[k].rt - dataArray[k].tarDelay + currentDelay/5 >= 0){
    //         //         continue;
    //         //     } else {
    //         //         break;
    //         //     }
    //         // }
    //         // console.log(dataArray[i].pts+"  "+dataArray[i].tarDelay + " " + dataArray[i].curDelay);
    //         // return dataArray[k - 1].pts + "  " + dataArray[k].pts;
    //         return dataArray[i].pts + "  " + peerClientStatusArr.backData[id].delayMST;
    //     }
    // }
    // if (i === dataArray.length) {
    //     if (dataArray[dataArray.length-1].pts - peerClientStatusArr.backData[id].delayMST - dataArray[i].pts < 1000 / dataArray[i].fps / 2) {
    //     }
    // }
    return dataArray[dataArray.length-1].pts - currentDelay;
}
