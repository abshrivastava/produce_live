...
var Signaling = function (signalingUrl) {
...
function setChannelEvents(channel,channelTag,peerName_,pc) {
    channel.onmessage = function (event) {
            ...
            if(data.video.recv["tracks"]&&data.video.recv["tracks"][0]){
                ...
                for(var i=0;i<data["results"].length;i++){
                    ...
                    if(result.indexOf("googTargetDelayMs")!==-1){
                        ...
                        peerClientStatusArr.backData[id].frameRate = data["results"][i]["googFrameRateOutput"]||25;
                        break; 
                    } 
                }
            }
        ...
        if (peerClientStatusArr.dataArray[peerName_].length < 90){
            peerClientStatusArr.dataArray[peerName_].push({pts: obj.ts, delay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(),fps:peerClientStatusArr.backData[peerName_].frameRate});
        } else {
            peerClientStatusArr.dataArray[peerName_].push({pts: obj.ts, delay: peerClientStatusArr.backData[peerName_].delayMSC, rt: Date.now(), fps:peerClientStatusArr.backData[peerName_].frameRate});
...
PeerClient.prototype.onRemoteStreamAdded = function(event) { //成功创建连接，获取到视频
    ...
    if(webrtcVideo.rtcIdList.pgm.id.indexOf(name)>-1){
        var div ='<video autoplay playsinline class="left main-player" id="'+name+'"></video>';
        var videoBox = $(".main-v .scale");
        videoBox.html(div);
        videoBox.find("video")[0].srcObject = event.stream;
		videoBox.find("video")[0].play();

        var pgmData = webrtcVideo.rtcIdList.pgm.data;
        videoBox.parents(".main-fn").find(".pgm-audio").attr("data-filename",pgmData["CurrentEncoderSharedMemory"]["SharedMemoryName"]);
        if($(".main-v .pgm-audio").hasClass("active")){
        ...
        webrtcVideo.rtcIdList.pgm.id = [];
        webrtcVideo.rtcIdList.pgm.data = null;
    ...
    }else if(webrtcVideo.rtcIdList.preLis.idArr.indexOf(name)>-1){
        var curItem = null,rid='',sourceTypeIcon='';
        webrtcVideo.rtcIdList.preLis.data.forEach(function(v,i){
        ...
        var previewListItem =   '<li class="single-sour sour" '+rid+' data-filename="'+curItem.PreviewShm.SharedMemoryName+'">\
                        <div class="sour-box">\
                            <video autoplay playsinline class="left main-player" muted id="'+name+'"></video>\
                        </div>\
                        <div class="sour-audio">\
                            <p class="v-title">Source 1</p>\
                            <div class="audio-body">\
                                <div class="a-monitor">\
                                <p class="erphone"><i class="iconfont icon-erphone"></i><span class="vol-size"></span></p>\
                                <div class="a-track">\
                                </div>\
                            </div>\
                            <div class="a-volume clearFix">\
                                <div class="vol-control left">\
                                </div>\
                                <div class="vol-show left">\
        ...
        //添加平衡滑块
        $("#"+name).parents(".single-sour").find(".a-track").sliderBalance();
        $("#"+name)[0].srcObject = event.stream;
        $("#"+name)[0].play();
        $("#"+name)[0].volume = 0;

        var itemEles = previewUlEle.find(".single-sour"); //根据peerId将视频排序
        ...
        eleRArr.forEach(function(v,i){
            $(v).find(".v-title").html("Source "+(i+1));
        ...
        videoDom.timer = setTimeout(function(){
            $.each($(".sour-ls").find("video"),function(idx,itm){
                itm.play();
            });
        },2000);
        ...
        var previewVolume = volumeColumn.volumeR2local(curItem.CurrentVolume);

        createVoiceControl($("#"+name).parents(".single-sour").find(".vol-control"),previewVolume);
        // var shareVolume = curItem.PreviewShm.SharedMemoryName;
        var id = webrtcVideo.rtcIdList.preLis.idArr.indexOf(name);
        if(id>-1){
            webrtcVideo.rtcIdList.preLis.idArr.splice(id, 1);
        }
        /*var curItem = null;
        webrtcVideo.rtcIdList.preLis.data.forEach(function(v,i){
            if(v.PreviewID==name) curItem = v;
...
var RtcClient = function (params,signalingUrl) {
    this.logIn = false;
    ...
    this.localStream_ = {};
    //signaling
    this.sigChannel_ = new Signaling(signalingUrl);
    this.initSigChannel();
    /*if (this.peerid_) {
...
function errorMsg(error) {
    ...
    if(err.indexOf("User not exists")>-1){
        ...
        if(videoObj.length!==0){
            ...
            if(webrtcVideo.rtcIdList.pvw.indexOf(error.name)>-1||webrtcVideo.rtcIdList.pgm.id.indexOf(error.name)>-1){
                clearTimeout(peerClientStatusArr.videoResetNumObjTime);
                peerClientStatusArr.videoResetNumObjTime = setTimeout(function(){
                    on_leave(error.name);
                    webrtcVideo.initFlag = true;
                    if(webrtcVideo.rtcIdList.pvw.indexOf(name)>-1){
                        webrtcVideo.rtcIdList.pvw = [];
                    }else{
                        webrtcVideo.rtcIdList.pgm.id = [];
                    }
                },2000);
                ...
                peerClientStatusArr.videoResetNumObjTime = setTimeout(function(){
                    ...
                    for(var j=0;j<userNotExists.length;j++){
                        ...
                        if(index>-1){
                            ...
                            webrtcVideo.rtcIdList.preLis.idArr.splice(index, 1);
                        }
                    }
...
function calPts(id) {
    ...
    for (i = 0; i < dataArray.length; i++) {
        if(currentT - dataArray[i].rt - dataArray[i].delay < 1000 / dataArray[i].fps) {
