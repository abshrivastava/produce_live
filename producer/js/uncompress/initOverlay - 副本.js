var initOverlay = (function($,oUtils,currentRInfo,pageRequestObj,createPack){
    function firstInitLogo(){
        ...
        oUtils.ajaxReq("logo_list",logoParams, function (data) {
    ...
    function initVideo () {
        ...
        if(saveIp.Ip != ip){ // 如果ip不一样触发overlay,恢复视频
            ...
            Object.keys(params).forEach(pos => {
                ...
                oUtils.ajaxReq("addExtAndSynchronize2R",{"extId":tid,"rId":rPeerId},function(data){
                    if(data.errorCode == '0x0'){
            ...
            oUtils.ajaxReq("saveUserBehavior", ipParam);
            // 恢复overlay
    ...
    function recoverLive(params, Liveparam, posistion, liveNum){
        ...
        oUtils.ajaxReq("startLive",Liveparam,function(data){
    ...
    function savePos(params,arr){
        ...
        oUtils.ajaxReq("saveUserBehavior", param, function(data){
            var param = {
                "pn": "sd"
            }
    ...
    function firstOverlyShow(){
        ...
        oUtils.ajaxReq("studio_list",pipParams, function (data) {
            if (data.errorCode == "0x0") {
    ...
    function firstClockInit(){
        ...
        var param = {};
        oUtils.ajaxReq("clock_list", param, function (data) {
            if (data.errorCode == "0x0") { 
               var  result=data.result;
                if(result!=""){
                    result=result[0];
    ...
    function initTlistWs(){
        var param=QRImg();
        //调接口获取二维码
        oUtils.ajaxReq("token_getToken4Pro",param,function(data) {
            var errorCode = data.errorCode;
            if( errorCode == "0x0" ) {
