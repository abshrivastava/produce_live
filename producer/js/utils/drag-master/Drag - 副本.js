	...
	$.xingquanDrag = {
		...
		handleTargetMoveEvent: function(e) {
			...
			$(document).off("mouseup").on("mouseup", function(e) {
				...
				if(params !== false){
					...
					}else if(logoObj.hasClass("logo-box")&&!$(logoObj).hasClass("textDiv")){
		...
		handleResizeEvent: function() {
			...
			$(this.settings.data.target).find('.point').off('mousedown').on('mousedown', function(e) {
				if(sourceList.isOrCanvas&&$(e.target).parent().hasClass("textDiv")){
					var opts = {
			            size: $("#preview .preTextarea").width()+"x"+$("#preview .preTextarea").height(),
			            bgcolor: $("#preview .preTextarea").css("background-color"), 
			            color: $("#preview .preTextarea").css("color"),
			            text: $("#preview .preTextarea").html(),
			            fstyle:'normal',
			            fweight: 'normal',
			            fsize:$("#preview .preTextarea").css("font-size"),
			            ffamily:$("#preview .preTextarea").css("font-family")
			        }
		            var base64 = placeholder.getData(opts);
		            $(".textDiv img").attr("src",base64);
		            $(".textDiv .preTextarea").css("display","none");
		            $(".textDiv img").css("display","block");
		        }
				...
				$(document).off("mouseup").on("mouseup", function(e) {
					...
					if($(".textDiv").lenght!==0){
						if(sourceList.isOrCanvas){
							...
							$(".textDiv .preTextarea").css("font-size",Math.floor(height*0.78)+"px");
							$(".textDiv .preTextarea").css("display","block");
			            	$(".textDiv img").css("display","none");
						}
						$(".textDiv").width("");
						$(".textDiv").height("");
					}
